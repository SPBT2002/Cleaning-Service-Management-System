...
import { deleteBooking as removeBooking, getBookingsSnapshot, subscribe, updateBooking } from '../../data/adminBookingsStore'
...
export default function AdminBookings(){
  ...
  const visible = bookings.filter(b=>{
    ...
    const matchesQuery = !q || b.name.toLowerCase().includes(q) || b.service.toLowerCase().includes(q)
  ...
  return (
    <AdminLayout>
      ...
      <div className="space-y-4">
        {visible.map(b=> (
          <div key={b.id} className="rounded-xl bg-[#0f2834] p-4">
            <div onClick={()=>toggleExpand(b.id)} className="flex items-center justify-between cursor-pointer">
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full bg-gradient-to-br from-[#11c1a8] to-[#3aa7ff] grid place-items-center text-white font-bold">{b.name.split(' ')[0][0]}</div>
                <div>
              ...
              <div className="text-right flex items-center gap-6">
                <div>
                  ...
                  <div className="text-sm text-[#90a0b2] mt-2 flex items-center gap-3 justify-end">
                    ...
                    <span className="inline-flex items-center gap-1"><svg className="h-4 w-4" viewBox="0 0 24 24" fill="none"><path d="M12 8v4l2 2" stroke="#9fb0c9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>{b.time}</span>
                  </div>
                </div>
            ...
            {expandedId===b.id && (
              <div className="mt-4 border-t border-[#14232b] pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={b.address} onChange={e=>updateBooking(b.id, {address: e.target.value})} className="w-full rounded-md bg-[#0b2130] border border-[#14232b] px-4 py-3 text-white" />
                </div>

                <div>
                  <label className="text-xs text-[#90a0b2] mb-2 block">Notes</label>
                  <textarea value={b.notes} onChange={e=>updateBooking(b.id, {notes: e.target.value})} rows={3} className="w-full rounded-md bg-[#0b2130] border border-[#14232b] px-4 py-3 text-white" />
                </div>

                <div className="md:col-span-2 flex items-center gap-3">
                  <div className="text-sm text-[#90a0b2] mr-2">Change status:</div>
                  {['Pending','Confirmed','Completed','Cancelled'].map(s=> (
                    <button key={s} onClick={()=>changeStatus(b.id, s)} className={`px-3 py-2 rounded-full text-sm ${b.status===s ? 'bg-[#223843] border border-[#2e4a52] text-white' : 'bg-[#0b2130] text-[#9fb0c9]'}`}>{s}</button>
                  ))}

                  <div className="ml-auto">
                    <button onClick={()=>deleteBooking(b.id)} className="px-3 py-2 rounded-md bg-transparent border border-[#3b2c2c] text-[#fda4af]">Delete</button>
                  </div>
                </div>
              </div>
            )}
